...
public class ResourceController : MonoBehaviour
{
    ...
    private void Start()
    {
        CurrentHealth = _statHandler.Health;//체력을 최대로 설정해주고
    }
    private void Update()
    {
        if (_timeSinceLastChange < healthChangeDelay)//마지막 체력변화시간보다 무적 시간이 길다면
        {
            _timeSinceLastChange += Time.deltaTime;//시간을 더해보고
            if (_timeSinceLastChange >= healthChangeDelay)
            {
                _animationHandler.InvincibilityEnd();//끝났다면 무적시간을 끝낸다.
            }
    ...
    public bool ChangeHealth(float change)
    {
        ...
        _timeSinceLastChange = 0.0f;//지나간 시간을 0초로 설정하고
